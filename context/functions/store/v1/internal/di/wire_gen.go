...
func Initialize() (*internal.Handler, error) {
	...
	dynamodbAnalysisRepository := repositories.NewDynamodbAnalysisRepository(awsDynamoDb, dotEnvEnvironmentReposity)
	dynamodbStatsRepository := repositories.NewDynamodbStatsRepository(awsDynamoDb, dotEnvEnvironmentReposity)
	handler := internal.NewHandler(logrusLogger, dynamodbAnalysisRepository, dynamodbStatsRepository)
	return handler, nil
}
